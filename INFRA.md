...
Sam is deployed on [Railway](https://railway.app). Use the `/deploy` skill for the full flow, or do it manually:
...
Set all `.env.local` values in Railway's **Variables** tab. Railway injects them at runtime — no `.env` file is deployed.

### Webhook URL after deploy

After first deploy, update the Meta webhook URL from your tunnel to the permanent Railway URL:
