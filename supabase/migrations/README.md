...
| `20260223_040000_match_spots_hybrid.sql` | New `match_spots_hybrid` RPC — combines all structured filters from `querySpots` with pgvector cosine similarity ranking; used by the hybrid query path when a query embedding is available | applied via MCP |
| `20260224_020000_add_coach_runs.sql` | New `coach_runs` table for autonomous self-coaching state (prompt snapshots for revert failsafe, avg_scores for trend tracking); add `coached_at` to conversations for incremental analysis | applied via MCP |
| `20260225_000000_add_confidence_score.sql` | Re-add `confidence_score integer` (0-100 completeness %) — distinct from old quality float dropped 20260220; populated by /api/batch-validate | applied via MCP |
| `20260303_000000_security_and_perf_fixes.sql` | Enable RLS on coach_runs + spot_corrections (service-role only); pin search_path on append_conversation_messages + increment_spot_contribution_count; add FK indexes for happenings.contributor_id + spot_corrections.spot_id; drop unused indexes (idx_spots_last_verified, idx_spot_contributions_contributor_id, idx_spots_avg_rating) | applied via MCP |
